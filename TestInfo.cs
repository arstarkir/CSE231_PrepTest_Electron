    ...
    internal class TestInfo
    {
        ...
        public void DissectTest(string filePath, string text)
        {
            ...
            for (int i = 0; i < numLines / 3; i++)
            {
                ...
                try
                {
                    int f = text.IndexOf(text.First(c => char.IsDigit(c) == true));
                ...
                for (int j = 0; j < nums.Count; j++)
                {
                    try
                    {
                        answers[int.Parse(nums[j])] = ans[j];
                        numOfQuestions = int.Parse(nums[j]);
                    }
                    catch
                    {
                        continue;
            ...
            for (int i = 1; i < numOfQuestions + 1; i++)
            {
                ...
                int questionNum = int.Parse(questionText.Substring(0, questionText.IndexOf(".")));
